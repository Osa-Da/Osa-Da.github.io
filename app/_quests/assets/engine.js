...
const Engine = (function () {
  ...
  // FIX: ищем запись кода и в персональных кодах персонажа, и в общих кодах библиотеки
  function findCodeEntry(name, typeFilter) {
    const own = Object.values(current.codeSystem || {}).find(
      (c) => c && c.type === typeFilter && c.name === name,
    );
    if (own) return own;
    const lib = LIBRARY && Array.isArray(LIBRARY.codes) ? LIBRARY.codes : [];
    return (
      lib.find((c) => c && c.type === typeFilter && c.name === name) || null
    );
  }
  ...
  // FIX: безопасное экранирование значений в inline-onclick (HTML + JS-строка)
  function escAttr(s) {
    return String(s ?? "")
      .replace(/&/g, "&amp;")
      .replace(/</g, "&lt;")
  ...
  function cardTemplate(ch) {
    return `
      ...
      <h2 class="text-xl sm:text-2xl font-semibold mb-3 flex items-center justify-end"><i data-lucide="users" class="mr-3"></i>${ch.faction ? ch.faction : "Описание вашей команды"}</h2>
  ...
  function mount(game, options) {
    ...
    if (savedCode && GAME.characters[savedCode])
      selectCharacter(savedCode, true);
  }
  ...
  function enterCode() {
    ...
    if (!GAME.characters[raw]) {
      showMsg("gateMsg", "Неверный код персонажа!", "error");
      return;
    }
    selectCharacter(raw);
  }
  ...
  function selectCharacter(code, silent) {
    ...
    current.quests.forEach((q) => {
      if (!quests.includes(q.title)) quests.push(q.title);
    });
  ...
  function renderFeatures() {
    ...
    const extra = unlockedNames
      .map((name) => findCodeEntry(name, "features")) // FIX: + LIBRARY.codes
      .filter(Boolean)
      .filter((f) => !base.some((b) => b.name === f.name))
      .map((f) => ({ ...f, isNew: true }));
  ...
  function renderAbilities() {
    ...
    const extra = unlockedNames
      .map((name) => findCodeEntry(name, "ability")) // FIX: + LIBRARY.codes
      .filter(Boolean)
      .filter((a) => !base.some((b) => b.name === a.name))
      .map((a) => ({ ...a, isNew: true }));
    const all = base.concat(extra);
    wrap.style.display = all.length ? "block" : "none";
    list.innerHTML = all
  ...
  function collectQuestItems() {
    ...
    const items = current.quests.map((q) => ({
      title: q.title,
      description: q.description,
      details: q.details,
      rewards: q.rewards,
      rewardsDescription: q.rewardsDescription,
    }));
    // FIX: + LIBRARY.codes — общие коды-задания тоже должны рендериться
    const libCodes =
      LIBRARY && Array.isArray(LIBRARY.codes) ? LIBRARY.codes : [];
    const allQuestCodes = Object.values(current.codeSystem || {}).concat(
      libCodes,
    );
    allQuestCodes.forEach((c) => {
      if (c && c.type === "quest" && unlocked.includes(c.name)) {
        items.push({
          title: c.name,
          description: c.content?.description,
          details: c.content?.details,
          rewards: c.rewards,
          rewardsDescription: c.rewardsDescription,
        });
      }
    });
    return items;
  ...
  function renderQuests() {
    ...
    list.innerHTML = items
      .map((q) => {
        ...
        const styleMap = {
          new: {
            cls: "border-emerald-strong tint-emerald-strong",
            badge: '<span class="badge badge-emerald">Новое</span>',
          },
          active: { cls: "border-emerald-dim tint-emerald-dim", badge: "" },
          done: {
            cls: "border-ultra-dim tint-ultra-dim",
            badge: '<span class="badge badge-emerald">Завершено</span>',
          },
          failed: {
            cls: "border-ultra-dim tint-ultra-dim",
  ...
  function renderInventory() {
    ...
    list.innerHTML = (current.inventory || [])
      .map(
        (it) => `
  ...
  function completeQuest(title, affected) {
    ...
    ["(Новое)", "(Провалено)"].forEach((suf) => {
      const i = arr.indexOf(title + " " + suf);
  ...
  function unlockAbility(name, affected) {
    const a = loadArr("unlockedAbilities");
    if (!a.includes(name)) {
      a.push(name);
      saveArr("unlockedAbilities", a);
    }
    affected.push({ type: "ability", name });
  }
  function unlockFeature(name, affected) {
    const a = loadArr("unlockedFeatures");
    if (!a.includes(name)) {
      a.push(name);
      saveArr("unlockedFeatures", a);
    }
    affected.push({ type: "feature", name });
  }
  function unlockQuest(name, affected) {
    const a = loadArr("unlockedQuests");
    if (!a.includes(name)) {
      a.push(name);
      a.push(name + " (Новое)");
      saveArr("unlockedQuests", a);
    }
    affected.push({ type: "quest", name });
  }

  function applyCode(codeData, messages, affected) {
    // FIX: защита от пустых кодов (type без name — не должен ничего разблокировать)
    if (!codeData || !codeData.name) return;
    const types = Array.isArray(codeData.type)
      ? codeData.type
      : [codeData.type];
    types.forEach((t) => {
      if (t === "ability") {
        unlockAbility(codeData.name, affected);
        messages.push(`Способность «${codeData.name}» разблокирована.`);
      } else if (t === "features") {
        unlockFeature(codeData.name, affected);
        messages.push(`Особенность «${codeData.name}» разблокирована.`);
      } else if (t === "quest") {
        unlockQuest(codeData.name, affected);
        messages.push(`Задание «${codeData.name}» разблокировано.`);
      }
    });
  }
  ...
  function processCode(raw) {
    ...
    current.quests.forEach((q) => {
      if (q.successfulCodes?.includes(raw)) {
        completeQuest(q.title, affected);
        messages.push(`Задание «${q.title}» завершено.`);
        matched = true;
      }
      if (q.failedCodes?.includes(raw)) {
        failQuest(q.title, affected);
        messages.push(`Задание «${q.title}» провалено.`);
        matched = true;
      }
    });
    Object.values(codeSystem).forEach((c) => {
      if (c.type === "quest") {
        if (c.successfulCodes?.includes(raw)) {
          completeQuest(c.name, affected);
          messages.push(`Задание «${c.name}» завершено.`);
          matched = true;
        }
        if (c.failedCodes?.includes(raw)) {
          failQuest(c.name, affected);
          messages.push(`Задание «${c.name}» провалено.`);
          matched = true;
        }
      }
    });

    // FIX: LIBRARY.codes — массив, ищем по key, а не индексируем строкой
    const libCode = Array.isArray(LIBRARY?.codes)
      ? LIBRARY.codes.find((c) => c.key === raw)
      : null;
    const codeData = codeSystem[raw] || libCode;
    if (codeData) {
      matched = true;
      if (codeData.targets && Array.isArray(codeData.targets)) {
        codeData.targets.forEach((t) => {
          if (codeSystem[t]) applyCode(codeSystem[t], messages, affected);
        });
  ...
  function highlightAffected(affected) {
    ...
    requestAnimationFrame(() => {
      ...
      affected.forEach((a) => {
        ...
        let el = null;
        nodes.forEach((n) => {
          if (n.getAttribute(attr) === a.name) el = n;
        });
  ...
  const POP_COLORS = {
    feature: "rgba(217,119,6,.55)",
    ability: "rgba(37,99,235,.55)",
    quest: "rgba(16,185,129,.55)",
  };

  function playSequential(els, i) {
    if (i >= els.length) return;
    const el = els[i];
    const category = el.hasAttribute("data-feature")
      ? "feature"
      : el.hasAttribute("data-ability")
        ? "ability"
        : "quest";
  ...
  function enterSecret() {
    ...
    if (messages.length) showMsg("secretMsg", messages.join("<br>"), "success");
      ...
      showMsg(
        "secretMsg",
        "Неверный код! Система зафиксировала попытку несанкционированного доступа.",
        "error",
      );
    input.value = "";
  }
  ...
  function renderDevSheet() {
    ...
    if (!current) {
      ...
      html += Object.keys(GAME.characters)
        .map((code) =>
          devItem(
            GAME.characters[code].name,
            code,
            // FIX: экранируем код для inline-onclick
            `Engine.devPickCharacter('${escAttr(code)}')`,
          ),
        )
        .join("");
    } else {
        ...
        ? entries
            .map(([codeKey, data]) =>
              devItem(
                data.name || codeKey,
                codeKey,
                `Engine.devApplyCode('${escAttr(codeKey)}')`, // FIX
              ),
            )
            .join("")
      ...
      const questCodes = [];
      current.quests.forEach((q) => {
        (q.successfulCodes || []).forEach((c) =>
          questCodes.push({ code: c, label: q.title + " — успех" }),
        );
        (q.failedCodes || []).forEach((c) =>
          questCodes.push({ code: c, label: q.title + " — провал" }),
        );
      });
          ...
          .map(
            (q) =>
              devItem(
                q.label,
                q.code,
                `Engine.devApplyCode('${escAttr(q.code)}')`,
              ), // FIX
          )
          .join("");
      }
      ...
      html += Object.keys(GAME.characters)
        .map((code) => {
          ...
          return devItem(
            GAME.characters[code].name,
            code,
            `Engine.devPickCharacter('${escAttr(code)}')`, // FIX
            active ? "border-blue-500 text-blue-300" : null,
          );
        })
        .join("");
    }
  ...
  function devPickCharacter(code) {
  ...
  function devApplyCode(codeKey) {
    ...
    setTimeout(() => {
      ...
      if (messages.length)
        showMsg("secretMsg", messages.join("<br>"), "success");
    }, 150);
  }
      ...
      list.innerHTML = items.length
        ...
        <div class="p-4 rounded-lg bg-black/20 border border-gray-700">
          ...
          ${g.mechanic ? `<div class="mt-2 p-2 bg-blue-900/20 rounded text-sm text-blue-300">${g.mechanic}</div>` : ""}
            ...
            .join("")
        : `<p class="text-gray-500 text-sm">Для этой игры справочник механик не задан.</p>`;
      document.getElementById("accountName").textContent = current
        ? current.name
        : "не выбран";
      refreshIcons();
    }
